#@# 最後の原稿生成時のコミット: 886b64bf6cb90d29f803ad262d8ff153a1dae7e7
...
初回起動時には、以下の手順に沿ってLightdashの初期化を行います。
 ...
 1. @<tt>{http://localhost:8080/}にアクセスし、必要な情報を入力して「Sign Up」ボタンを押します
 2. 管理者の情報を入力して「Next」ボタンを押します
 3. データウェアハウスの選択画面では、PostgreSQLを選びます
 4. dbtプロジェクトのアップロード方法については、「Using your CLI」を選択します
 5. 画面に表示されたコマンドをLightdashコンテナの中で実行します
 6. 初期化が完了したことを確認します
...
以下の手順は、Dockerコンテナが動作している状態で実行してください。
 ...
 1. @<tt>{demo}ディレクトリに移動します
 2. @<tt>{uv run init.py}を実行します
 3. @<tt>{uv run seed.py}を実行します
...
実行方法は以下の通りです。
 ...
 1. @<tt>{dbt_project/seeds_loader}ディレクトリに移動します
 2. @<tt>{uv run init.py}を実行します
 3. @<tt>{uv run load.py}を実行します
...
ローカルPCから変換を実行する場合は、以下の手順に沿って実行してください。
 ...
 1. @<tt>{dbt_project}ディレクトリに移動します
 2. @<tt>{uv run dbt run --target local}を実行します

これで、Lightdashで参照するデータが、データウェアハウス上に作成されました。

== Lightdashの使い方

Lightdashの基本的な使い方は次の通りです。

 1. クエリを作成します
 2. クエリを元にチャートを作成します
 3. 複数のチャートを組み合わせてダッシュボードを作成します

ここで登場する用語を簡単に整理します。

 * クエリ
どのテーブルのどの列を使って集計するかを定義したものです。dbtで定義したモデルをベースに、Lightdashの画面から列を選んで作成します。

 * チャート
クエリの結果を棒グラフや折れ線グラフなどで可視化したものです。Lightdashではクエリと同じ画面でチャートを作成します。

 * ダッシュボード
複数のチャートを一画面にまとめたものです。日々のモニタリングや定期レポートに活用します。

=== クエリの作成方法

クエリを作成するときは、画面左上の「New」メニューから「Chart」を選択します。Lightdashではクエリとチャートは同一画面で作成するため、この操作からスタートします。

//image[chapter01/query_1_choise_table][テーブルの選択]{
//}

まずクエリの基準となるテーブルを一つ選びます。スタースキーマの中心に位置するファクトテーブルを選ぶのが基本です。

===[column] 表示テーブルの絞り込み

画面左側のテーブル一覧には、マート層以外のstaging層などのモデルも表示されることがあります。その場合は「Project Settings」→「Table Configuration」から、表示するテーブルを絞り込むことができます。

===[/column]

//image[chapter01/query_2_choise_columns][列の選択]{
//}

ここでは「購入（fct_purchase）」テーブルを選びます。するとテーブルに含まれる項目の一覧が表示されるので、以下の2つを選択します。列名をクリックすると選択状態になります。

 * 購入 / Dimensions / 購入日時 / Monthly
 * 購入 / Metrics / 売上合計

選択したら「Run Query」ボタンをクリックします。クエリが実行され、結果がテーブル形式で表示されます。列名の横の矢印をクリックすると、並び順を変えるメニューが表示されます。

//image[chapter01/query_3_query_result][クエリの実行結果]{
//}
...
クエリ結果の表の上にある「Chart」カテゴリを開くと、棒グラフが自動的に表示されます。Lightdashではクエリを作成すると、棒グラフのチャートが自動的に生成されます。

//image[chapter01/chart-1-default][自動生成されたチャート]{
//}

「Configure」ボタンをクリックすると、グラフの種類や表示設定を変更する画面が表示されます。グラフの種類の変更、スケールや凡例の説明の変更など、さまざまなカスタマイズができます。

//image[chapter01/chart-2-settings][チャートの設定画面]{
//}

設定が完了したら、「Save Chart」ボタンをクリックしてクエリとチャートを保存します。

//image[chapter01/chart-3-save][チャートの保存][scale=0.5]{
//}

=== ダッシュボードの作成方法

ダッシュボードを作成するときは、画面左上の「New」メニューから「Dashboard」を選択します。

//image[chapter01/dashboard-1-input-name][ダッシュボード名の入力][scale=0.5]{
//}
...
セマンティックレイヤーとは、データウェアハウスと利用者の間に位置し、技術的なテーブル定義をビジネス上の概念に変換する層のことです。たとえば @<tt>{subtotal} というカラム名を「売上合計」という言葉に変換したり、合計や平均などの集計ロジックを一度定義して組織全体で再利用できるようにします。これにより、データチームがメトリクスを一元管理でき、部署によって計算方法が異なるといった問題を防ぎやすくなります。

Lightdashはdbtモデルのセマンティックレイヤーと連携する設計になっています。Lightdashにおけるセマンティックレイヤーは次の3つの要素で構成されます。

 * メトリクス
売上合計や購入件数など、ビジネスが注目する数値の集計定義です。一度定義すれば、組織全体で統一された計算が保証されます。

 * ディメンション
購入日時やカテゴリ名など、メトリクスを分析するための属性（切り口）です。メトリクスをどの軸で見るかを定義します。

 * テーブル
分析の基盤となるビジネスオブジェクト（購入・会員・食品など）の単位です。

これらはdbtモデルのYAMLファイルの @<tt>{meta} フィールドに記述することでLightdashに取り込まれます。もう一つの方法として独自のYAMLファイルを使う方法もありますが、本書ではdbtプロジェクトが既に存在するため、モデル定義への追記方式を採用しています。

以降では、購入モデル（@<tt>{fct_purchase}）を例に使って説明します。
...
明細件数	count	id	購入明細の総件数
売上合計	sum	subtotal	購入明細の小計合計
平均小計	average	subtotal	購入明細の平均小計
販売数量合計	sum	quantity	数量の合計
...
//image[chapter01/semanticlayer-1-example-metrics-model][モデルに定義したメトリクス]{
//}
...
//image[chapter01/semanticlayer-2-example-metrics-column][列に定義したメトリクス]{
//}
...
ドリルダウンの手順は次の通りです。
 ...
 1. ドリルダウンしたいデータを、チャート上でクリックします。
 2. ドリルダウンで分析したい列を選択します。
 3. 見やすいチャートを選択します。

まず初めに、ドリルダウンしたいデータを、チャート上でクリックします。ここでは、2025/8の売り上げデータの内訳をドリルダウンしようとしています。

//image[chapter01/drilldown-1-choice_bar][ドリルダウンするデータの選択]{
//}

次に、どの列単位で内訳を見たいかを選択します。ここでは、商品のカテゴリごとに内訳を確認することにします。

//image[chapter01/drilldown-2-choise_column][分析列の選択]{
//}

最後に出てきた結果を見やすいチャートに変換します。ドリルダウンして新しく表示されたチャートでは、あらかじめfilterで選択したデータだけが抽出されるようになっています。ここでは棒グラフを円グラフに変換して、内訳をみやすくしています。
